// ============================================================================
// AUTH SERVICE
// Handles login, registration, SSO, and token refresh for EMP Exit.
...
export async function register(data: RegisterData): Promise<LoginResult> {
  ...
  // For exit module, registration creates a user in an existing org context.
  // In a full setup, org creation is handled by EmpCloud. Here we validate
  // and create the user record directly.
  ...
  const payload: AuthPayload = {
    ...
    orgName: org?.name || data.orgName,
  };

  const accessToken = signAccessToken(payload);
  const refreshToken = signRefreshToken(userId);

  logger.info(`New organization registered: ${data.orgName} by ${data.email}`);
...
/**
 * SSO login: exchange an EMP Cloud RS256 JWT for an Exit-specific HS256 JWT.
